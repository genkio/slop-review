...
import { planSync, mapGithubThreadToSlop, runSync } from '../server/sync.js'
...
test('planSync: locally_modified is skipped, never updated (even when still unresolved)', () => {
  const local = [{ id: 'thread_1', github_thread_id: 'A', locally_modified: true }]
  const { toUpsert, skippedModified } = planSync([{ id: 'A' }], local)
  assert.equal(skippedModified, 1)
  assert.equal(toUpsert.length, 0)
})
